...
void Server::sendCommand(const QVariant &data, const Enums::Commands command) {
  ...
  case Enums::Commands::ComFillScreen: {
    ...
    emit sendMessage("Сервер отправил команду заливки на " + IP_ADDR.toString(),
                     Enums::Messages::Send);
    break;
  }
  case Enums::Commands::ComSendText: {
    ...
    m_pudp->writeDatagram(packet, IP_ADDR, UDP_PORT);
    emit sendMessage("Сервер отправил данные на " + IP_ADDR.toString(),
                     Enums::Messages::Send);
    break;
  }
  default:
...
void Server::onReadyRead() {
  while (m_pudp->hasPendingDatagrams()) {
    ...
    m_pudp->readDatagram(datagram.data(), datagram.size(), &sender,
                         &senderPort);
...
QByteArray Server::resolveDnsAns(QByteArray &query) {
  ...
  header.id =
      static_cast<quint8>(query[0]) << 8 | static_cast<quint8>(query[1]);
  query.remove(0, 2);
  header.flags =
      static_cast<quint8>(query[0]) << 8 | static_cast<quint8>(query[1]);
  query.remove(0, 2);
  ...
  case 0x0010: { // TXT record type
    ...
    answer.txt =
        QByteArray(reinterpret_cast<const char *>(text.size()), 1).append(text);
    answer.dataLength = answer.txt.size();
    break;
  }
  default: {
